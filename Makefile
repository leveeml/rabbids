...
integration: ## Run all the integration tests
	go test $(TEST_OPTIONS) -covermode=atomic -coverprofile=coverage.txt -integration -timeout=5m -cover -json $(SOURCE_FILES) | $$(go env GOPATH)/bin/tparse -top -smallscreen -all -dump
